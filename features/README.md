...
```
features/
  00-project-setup/         ← Do first. Blocks everything else.
  01-inventory-management/  ← Do second. No dependencies on other features.
  02-customer-management/   ← Do in parallel with 01.
  03-checkout-and-receipt/  ← Depends on 01 + 02.
  04-return-and-invoice/    ← Depends on 03 + 05.
  05-configuration/         ← Do early (late fee rules seed on startup).
  06-reporting/             ← Do last. Read-only queries on existing data.
```
...
| Decision | Choice |
|----------|--------|
| All monetary amounts | Stored in **paise (integer)**. Display: divide by 100 |
