...
| 1.5 | August 9, 2026 | Public photo gallery epic (#52) started. Gallery category source decided (ADR-011): `gallery_images.category` reuses the `Item.Category` enum with no DB CHECK constraint, so new categories require only an enum edit (no migration). |
...
### ADR-011: Gallery Images Reuse the `Item.Category` Enum, Without a DB CHECK Constraint

| Field | Value |
|-------|-------|
| **Status** | Accepted |
| **Context** | The public photo gallery (epic #52) stores standalone marketing images grouped by category, separate from live rental inventory. US-53 (gallery data model) must decide the category source. Three options were weighed: (1) reuse the existing `com.fashionrental.inventory.Item.Category` enum, (2) a new gallery-only enum, (3) a free-text `VARCHAR` category. The epic describes uploads as "folder-style," implying the owner may want to introduce new categories over time without a code deploy. |
| **Decision** | The `gallery_images.category` column reuses the **`Item.Category` Java enum** (`@Enumerated(STRING)`), so the gallery shares the shop's established thematic taxonomy (`TRADITIONAL`, `MYTHOLOGICAL`, `FREEDOM_FIGHTER`, `PROFESSIONS`, `FANCY_DRESS`, `SEASONAL`, etc.) rather than forking a duplicate list. However, **no DB `CHECK` constraint** is placed on the column — it is a plain `VARCHAR(20) NOT NULL`. Adding a new category later requires only appending a value to the `Item.Category` enum, with no Flyway migration to alter a constraint. |
| **Rationale** | **Reuse over a new enum:** the seven thematic values a gallery would need already exist in `Item.Category` (added in `V20260708001`). A separate enum would duplicate them and drift over time (a category added to one list but not the other), confusing customers who see a gallery category with no matching inventory filter. **No CHECK constraint:** a `CHECK (category IN (...))` would mean every new category needs both an enum edit *and* a migration to alter the constraint — friction for a single non-technical owner. Dropping the constraint keeps the enum as the single source of truth while letting categories grow with a one-line code change. Type safety is retained in Java (the enum), and `ddl-auto: validate` is unaffected (it does not check CHECK constraints). **Free-text VARCHAR rejected (for now):** while it would allow zero-code category creation, it sacrifices the type-safe filter contract the frontend (US-55) relies on and risks casing duplicates (`Wedding` vs `wedding`). |
| **Consequences** | Gallery categories are bounded by the `Item.Category` enum, shared with inventory — a deliberate coupling at the taxonomy level (not the data level; gallery images remain standalone marketing assets). Introducing a gallery-only category that should *not* appear in inventory is not possible without revisiting this decision. Because there is no DB CHECK, an out-of-enum string could in principle be inserted via raw SQL, but all application writes go through the JPA enum mapping, which enforces the constraint at the application boundary. `GalleryImageMappingIT` persists a row for every enum value against real PostgreSQL, guarding that each category round-trips and that `@PrePersist` populates `created_at`/`updated_at`. |
